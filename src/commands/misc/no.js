...
module.exports = {
    ...
    execute: async (message, args) => {
        ...
        if (args.length === 0) return channel.send(`What are you trying to say buddy?`)
        ...
        try {
            phin({url}).then(async res => {
                ...
                fs.writeFile(`${imagePath}/${args[0]}.png`, res.body, async (err) => {
                    ...
                    const reply = await channel.send({ embeds: [embed], files: [file] })
                })
            })
        } catch (error) {
            console.log({error});
            const reply = await channel.send(`I'm calling the police`)
        }
    }
}